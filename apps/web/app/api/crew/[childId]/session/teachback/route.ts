...
import { answerTeachback } from '@/lib/crew/orchestrator';
...
export async function POST(request: Request, { params }: { params: Promise<{ childId: string }> }) {
  ...
  const parsed = z
    .object({
      ...
      correctionIndex: z.number().int().min(0).max(5),
  ...
  try {
    return NextResponse.json(await answerTeachback(childId, parsed.data));
  } catch (error) {
